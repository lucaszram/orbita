...
**Cambios:** `app.json` (buildNumber 40) y el test que fija el número
...
(aditivos) y la reconciliación de CORE-247. Backend de producción ya desplegado desde el mismo
commit (compatible con el 28).

## main contiene a release/1.0.0 (CORE-247, 2026-09-06)

**Objetivo:** una sola línea. Nativo = release/1.0.0 (`1f9c57a`, build 28);
web = main (`fa04d8e`, paridad Figma); backend = unión. Base común `52836ad`.
Merge de `origin/release/1.0.0` sobre `origin/main` en la rama
`orb/reconciliacion-release-main`, sin reescribir historia.

**Decisiones archivo por archivo (21 conflictos):**
- Rutas → release (envoltorios): `app/(tabs)/_layout.tsx`, `app/(tabs)/vinculo.tsx`,
  `app/reading/transito.tsx`, `app/reading/vinculo-result.tsx`, `src/routes/v492/diario.web.tsx`
  (rename espurio con `app/umbral.tsx`, que release borra).
- Variantes web reescritas con la pantalla de main: `src/routes/v492/tabs-vinculo.web.tsx`
  (`VinculosScreen`), `reading-transito.web.tsx` (detalle CORE-208), `reading-vinculo-result.web.tsx`
  (comparación CORE-236), `tabs-layout.web.tsx` (`navKeyForPath` de main). Umbral web con Tarot:
  `app/(tabs)/umbral/index.tsx` pasa a envoltorio con `tabs-umbral.tsx` (nativo, VoidExperience) y
  `tabs-umbral.web.tsx` (`UmbralSections`).
- Compartidos: `kit.tsx` unión de imports; `VoidExperience.tsx` unión de props (`sectionLabel`/`belowHeader`
  de main + `suggestionsFailed`/`onRetrySuggestions` de release) y `{belowHeader}` sobre la envoltura
...
- Backend: `relationships.ts` unión de imports y de constantes (`SIGNOS_ES` para `addPerson`; release resuelve
  ...
  `_generated` regenerado con `pnpm convex:codegen`; `CHANGELOG.md` unión cronológica.
...
la explicación del orden sigue la de CORE-207 (pesos por planeta, punto y
aspecto), que es lo que el backend hace; el frame la escribe con otros
rótulos.
