...
function navKeyForPath(pathname: string): NavKey {
  ...
  // Vínculo es sección de la barra web (CORE-212): la ruta existe y se entra,
  // así que tiene que poder marcarse.
  if (pathname.startsWith("/vinculo")) return "vinculo";
  if (pathname.startsWith("/vacio") || pathname.startsWith("/umbral")) return "umbral";
  // Perfil no es sección de la barra web (CORE-239: es la píldora PERFIL de la
  // cabecera): en la barra se marca Carta, que es desde donde se entra.
