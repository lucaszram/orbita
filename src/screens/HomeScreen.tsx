 ...
 *
 * Vivía dentro de `app/(tabs)/index.tsx`, así que la web tenía que mantener su
...
export function HomeScreen() {
  ...
  // Antes salía de `toLocalDate()` (reloj del dispositivo): con una zona natal
  // distinta a la del aparato, la guía y la carta del día podían pedirse para
  // un día que no era el del ritual. `null` mientras resuelve → no se consulta.
  ...
  const { state: dailyState, retry: retryDaily } = useDailyGuide(today ? userKey : null, today ?? "", isAuthLoading);
  ...
  // La ventana del historial se deriva de la fecha canónica, no del reloj.
  const stripDays = useMemo(() => (today ? lastNDaysFrom(7, today) : []), [today]);
  const stripQ = useQuery(
    proposedApi.dailyStrip,
    isLive && stripDays.length ? { from: stripDays[0], to: stripDays[stripDays.length - 1] } : "skip"
  );
  ...
  async function pullCard(): Promise<boolean> {
    ...
    // Sin fecha canónica no se revela: el reveal es irreversible y no puede
    // caer sobre un día adivinado por el cliente.
