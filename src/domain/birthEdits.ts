...
/**
 * ¿Se puede guardar ya? Con sesión iniciada hay que ESPERAR a que resuelva la
 * query de birthData remota: si el lugar no cambió y el doc todavía no llegó,
 * `buildBackendBirthPayload` mandaría timezone undefined y el backend caería a
 * la timezone del teléfono, recalculando mal una carta nacida en otra zona.
 */
export type BirthSaveGate = "ok" | "wait-remote";

export function birthSaveGate(input: { signedIn: boolean; remoteResolved: boolean }): BirthSaveGate {
  return input.signedIn && !input.remoteResolved ? "wait-remote" : "ok";
}

/**
 * Estado visible de esa espera: la protección de la timezone se mantiene
 * (nunca se guarda sin el doc remoto), pero la espera NUNCA es infinita —
 * pasado el timeout se muestra error con reintento.
 */
export type BirthSyncUx = "ready" | "waiting" | "retry";

export function birthSyncUx(gate: BirthSaveGate, timedOut: boolean): BirthSyncUx {
