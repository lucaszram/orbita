...
describe("birthSaveGate — no recalcular la carta con la timezone del teléfono", () => {
  it("con sesión y birthData remoto sin resolver → esperar (no guardar)", () => {
    assert.equal(birthSaveGate({ signedIn: true, remoteResolved: false }), "wait-remote");
  });

  it("con sesión y birthData resuelto (aunque sea null) → guardar", () => {
    assert.equal(birthSaveGate({ signedIn: true, remoteResolved: true }), "ok");
  });

  it("invitado → guarda local sin esperar nada", () => {
    assert.equal(birthSaveGate({ signedIn: false, remoteResolved: false }), "ok");
    assert.equal(birthSaveGate({ signedIn: false, remoteResolved: true }), "ok");
  });
});

describe("birthSyncUx — la espera del remoto nunca es infinita", () => {
  it("esperando dentro del tope → waiting (Guardar bloqueado)", () => {
    assert.equal(birthSyncUx("wait-remote", false), "waiting");
  });

  it("tope vencido → retry (error visible + reintento), sin aflojar la protección", () => {
    assert.equal(birthSyncUx("wait-remote", true), "retry");
  });
