 ...
 *
 * De qué base es el ORÁCULO
 * -------------------------
 * `lecturaAmplia` es la copia literal del código que este árbol tenía ANTES de
 * la tarjeta: el `by_user` + `collect`, el `.filter()` del alcance diario y el
 * `.sort()` por `updatedAt`, tal como estaban en `convex/layers.ts` en
 * `ad5ac90`. No se toma de ninguna otra rama ni se deriva del código nuevo —eso
 * sería circular—, y ningún `expected` de este archivo se escribe a mano para
 * que la comparación cierre: el esperado se calcula corriendo ese oráculo sobre
 * el mismo corpus.
 *
 * Qué tiene este árbol que la referencia no
 * -----------------------------------------
 * Acá conviven Free y Órbita Plus: `getForDateWithAccess`,
 * `getTransitArcWithAccess`, `refreshForDateWithAccess` y
 * `refreshTransitArcWithAccess` envuelven a los mismos handlers con
 * `enforcePlan`, `getRefreshState` publica `isPro`, y el refresco Free persiste
 * sólo `ORB-LUN-001` y `ORB-NAT-001`. El alcance de lectura no sabe nada de
 * planes, así que acotar la lectura no puede mover una gate: la sección 7 lo
 * comprueba sobre los wrappers REALES, con una cuenta Free y una cuenta Plus,
...
function baseInstrumentada() {
  ...
  const db = {
    /**
     * Escrituras. Existen para que el ciclo de refresco Free corra ENTERO
     * —`refreshForDateWithAccess` → `persistRefresh`— contra esta misma base, y
     * para que lo que ese ciclo persiste se pueda volver a leer por el alcance
     * nuevo. No se contabilizan como lectura: lo que mide este archivo es lo
     * que cuesta LEER.
     */
    async insert(nombreTabla: string, campos: Record<string, unknown>) {
      return seed(nombreTabla, campos);
    },
    async patch(id: string, campos: Record<string, unknown>) {
      for (const filas of tablas.values()) {
        const fila = filas.find((otra) => otra._id === id);
        if (!fila) continue;
        Object.assign(fila, structuredClone(campos));
        return;
      }
      throw new Error(`no existe la fila ${id}`);
    },
    async get(id: string) {
      for (const filas of tablas.values()) {
...
// ---------------------------------------------------------------------------
// 7. La divergencia de esta rama: Free y Órbita Plus sobre el alcance acotado
// ---------------------------------------------------------------------------

/**
 * Lo que esta sección tiene que dejar demostrado.
 *
 * El alcance de lectura no sabe nada de planes: `scopedAnalysisSnapshots` y
 * `natalAnalysisSnapshots` traen las mismas filas para Free y para Órbita Plus,
 * y el corte sigue viviendo donde vivía —`enforcePlan` en `getForDateForPlan`,
 * `getTransitArcForPlan` y `refreshForDateForPlan`—. Eso NO se da por supuesto
 * acá: se corre el wrapper real con una cuenta Free y con una cuenta Plus, y se
 * mira lo que publica.
 *
 * La cuenta Plus se arma con una fila de `subscriptions` sintética, de
 * `provider: "stripe"`. Stripe es el único proveedor que `isRowActive` resuelve
 * sin mirar el entorno del deployment, así que la prueba no depende de ninguna
 * variable de entorno, de ninguna allowlist ni de ningún proveedor de pagos: es
 * un dato sembrado, como todos los demás de este archivo.
 */
function activarPlus(escenario: Escenario) {
  return escenario.base.seed("subscriptions", {
    userId: escenario.userId,
    entitlement: "orbita_pro",
    status: "active",
    provider: "stripe",
    plan: "monthly",
    // Bien después de `INSTANTE`, que es el reloj de estas comparaciones.
    currentPeriodEnd: Date.parse("2027-01-01T00:00:00.000Z"),
    updatedAt: 1,
  });
}

const leerDiaConAcceso = (
  escenario: Escenario,
  localDate: string,
  timezone: string,
  db?: any,
): Promise<SobreDelDia> =>
  (getForDateWithAccess as any)._handler(escenario.ctx(db), { localDate, timezone });

const leerArco = (escenario: Escenario, arcId: string, db?: any): Promise<AnalysisResult> =>
  (getTransitArc as any)._handler(escenario.ctx(db), { localDate: HOY, timezone: ZONA, arcId });

const leerArcoConAcceso = (
  escenario: Escenario,
  arcId: string,
  db?: any,
): Promise<AnalysisResult> =>
  (getTransitArcWithAccess as any)._handler(escenario.ctx(db), {
    localDate: HOY,
    timezone: ZONA,
    arcId,
  });
...
/**
 * El ctx de una action, con los dos saltos REALES.
 *
 * `runQuery` y `runMutation` no devuelven un estado inventado: ejecutan los
 * handlers de producción —`getRefreshState` y `persistRefresh`— contra esta
 * misma base instrumentada. Así el ciclo de refresco Free se observa de punta a
 * punta (qué lee, qué decide y qué escribe) en vez de afirmarse sobre un doble
 * que ya contesta lo que la prueba quiere oír.
 */
function ctxDeAccion(escenario: Escenario) {
  const mutaciones: Array<Record<string, any>> = [];
  return {
    mutaciones,
    ctx: {
      auth: { getUserIdentity: async () => ({ tokenIdentifier: TOKEN }) },
      runQuery: async (_referencia: unknown, args: any) =>
        (getRefreshState as any)._handler(escenario.ctx(), args),
      runMutation: async (_referencia: unknown, args: any) => {
        mutaciones.push(args);
        return (persistRefresh as any)._handler(escenario.ctx(), args);
      },
    },
  };
}

/** Los sobres que `lockedLayerBundle` cierra, y los dos natales que deja abiertos. */
const CERRADOS_PARA_FREE: Array<[string, (sobre: SobreDelDia) => AnalysisResult]> = [
  ["natal.relationshipPattern", (bundle) => bundle.natal.relationshipPattern],
  ["today.transitRanking", (bundle) => bundle.today.transitRanking],
  ["today.transitArc", (bundle) => bundle.today.transitArc],
  ["today.moonOnChart", (bundle) => bundle.today.moonOnChart],
  ["today.cumpleluna", (bundle) => bundle.today.cumpleluna],
  ["moment.progressedLunation", (bundle) => bundle.moment.progressedLunation],
  ["moment.annualProfection", (bundle) => bundle.moment.annualProfection],
  ["moment.temporalMandala", (bundle) => bundle.moment.temporalMandala],
];

const ABIERTOS_PARA_FREE: Array<[string, (sobre: SobreDelDia) => AnalysisResult]> = [
  ["natal.lunarType", (bundle) => bundle.natal.lunarType],
  ["natal.elementMap", (bundle) => bundle.natal.elementMap],
];

test("el inventario de exports de convex/layers.ts no cambia con la tarjeta", async () => {
  const { readFileSync } = await import("node:fs");
  const fuente = readFileSync(`${process.cwd()}/convex/layers.ts`, "utf8");
  const exportados = [...fuente.matchAll(/^export (?:const|function) (\w+)/gm)].map(
  ...
  assert.deepEqual(
    ...
    "CORE-431 acota lecturas: no agrega, no saca y no renombra ninguna función pública",
  );
});

test("getForDateWithAccess cierra las capas de pago para Free y las abre para Plus, con el mismo alcance", async () => {
  const escenario = cuenta();
  await sembrarCacheRealista(escenario, HOY, ZONA);
  sembrarRuido(escenario, { dias: 40, zonas: [ZONA, ZONA_AJENA] });

  // --- Free -----------------------------------------------------------------
  const libre = await medir(escenario, (db) => leerDiaConAcceso(escenario, HOY, ZONA, db));
  anotar("getForDateWithAccess · Free", libre);
  assert.deepEqual(
    libre.respuestaDespues,
    libre.respuestaAntes,
    "acotar la lectura no puede cambiar lo que ve una cuenta Free",
  );
  for (const [nombre, tomar] of CERRADOS_PARA_FREE) {
    const cerrado = tomar(libre.respuestaDespues);
    assert.deepEqual(cerrado.missingInputs, ["orbita_plus"], `${nombre} tiene que quedar cerrado`);
    assert.equal(cerrado.data, null, `${nombre} no puede publicar dato para Free`);
    assert.equal(cerrado.status, "unavailable", `${nombre} tiene que declararse no disponible`);
  }

  // Y lo que Free SÍ recibe sale de la lectura acotada, no de un sobre vacío:
  // es exactamente el mismo natal que publica la lectura sin gate.
  const sinGate = await enElInstante(INSTANTE, () => leerDia(escenario, HOY, ZONA));
  for (const [nombre, tomar] of ABIERTOS_PARA_FREE) {
    assert.deepEqual(
      tomar(libre.respuestaDespues),
      tomar(sinGate),
      `${nombre} tiene que seguir llegando entero a Free`,
    );
    assert.equal(
      tomar(libre.respuestaDespues).missingInputs.includes("orbita_plus"),
      false,
      `${nombre} no es una capa de pago`,
    );
  }

  // --- Plus -----------------------------------------------------------------
  activarPlus(escenario);
  const pago = await medir(escenario, (db) => leerDiaConAcceso(escenario, HOY, ZONA, db));
  anotar("getForDateWithAccess · Órbita Plus", pago);
  assert.deepEqual(
    pago.respuestaDespues,
    pago.respuestaAntes,
    "acotar la lectura no puede cambiar lo que ve una cuenta Plus",
  );
  assert.deepEqual(
    pago.respuestaDespues,
    sinGate,
    "con Plus, el wrapper publica exactamente lo mismo que la lectura sin gate",
  );
  assert.equal(pago.respuestaDespues.today.transitRanking.status, "ready");
  assert.notDeepEqual(
    pago.respuestaDespues,
    libre.respuestaDespues,
    "si Free y Plus publicaran lo mismo, la gate no estaría cortando nada",
  );

  // --- El alcance: el mismo para los dos planes, y acotado en los dos --------
  assert.deepEqual(
    libre.despues,
    pago.despues,
    "el alcance de lectura no depende del plan",
  );
  assert.equal(libre.despues.documentos, 9);
  assert.ok(libre.despues.documentos < libre.antes.documentos / 10);
  assert.ok(pago.despues.documentos < pago.antes.documentos / 10);
});

test("getTransitArcWithAccess corta Free sin tocar el caché, y Plus sigue sirviendo el arco guardado", async () => {
  const escenario = cuenta();
  await sembrarCacheRealista(escenario, HOY, ZONA);
  const observedAt = 1_700_000_000_000;

  // El hash del arco pedido lo publica el propio handler: no se recalcula acá.
  const sinCalcular = await enElInstante(INSTANTE, () =>
    leerArco(escenario, RANKING_TOP.arcId),
  );
  assert.equal(sinCalcular.analysisId, "ORB-TRN-001");
  escenario.base.seed(
    "analysisSnapshotsV492",
    filaDeSnapshot({
      userId: escenario.userId,
      result: sobre({
        analysisId: "ORB-TRN-001",
        inputHash: sinCalcular.inputHash,
        observedAt,
        data: datosDeArco(observedAt),
      }),
      localDate: HOY,
      timezone: ZONA,
      updatedAt: observedAt,
  ...
  const libre = await enElInstante(INSTANTE, () =>
    leerArcoConAcceso(escenario, RANKING_TOP.arcId),
  );
  assert.deepEqual(libre.missingInputs, ["orbita_plus"], "Free no recibe el arco");
  assert.equal(libre.data, null);

  activarPlus(escenario);
  const pago = await enElInstante(INSTANTE, () =>
    leerArcoConAcceso(escenario, RANKING_TOP.arcId),
  );
  assert.equal(arcoPublicado(pago)?.arcId, RANKING_TOP.arcId, "Plus recibe el arco guardado");
  assert.equal(
    pago.inputHash,
    libre.inputHash,
    "la gate decide si se publica, no cambia la identidad de caché del arco",
  );
  assert.deepEqual(
    pago,
    await enElInstante(INSTANTE, () => leerArco(escenario, RANKING_TOP.arcId)),
    "con Plus, el wrapper publica lo mismo que la lectura sin gate",
  );
});

test("getRefreshState sigue publicando isPro, y el alcance acotado no lo mueve", async () => {
  const escenario = cuenta();
  await sembrarCacheRealista(escenario, HOY, ZONA);
  sembrarRuido(escenario, { dias: 40, zonas: [ZONA, ZONA_AJENA] });

  const libre = await medir(escenario, (db) => leerRefresco(escenario, HOY, ZONA, db));
  assert.equal(
    (libre.respuestaDespues as any).isPro,
    false,
    "sin suscripción activa, el estado del refresco tiene que decir Free",
  );
  assert.equal((libre.respuestaAntes as any).isPro, false);
  assert.deepEqual(libre.respuestaDespues, libre.respuestaAntes);

  activarPlus(escenario);
  const pago = await medir(escenario, (db) => leerRefresco(escenario, HOY, ZONA, db));
  assert.equal(
    (pago.respuestaDespues as any).isPro,
    true,
    "con suscripción activa, el estado del refresco tiene que decir Plus",
  );
  assert.equal((pago.respuestaAntes as any).isPro, true);
  assert.deepEqual(pago.respuestaDespues, pago.respuestaAntes);

  // El plan cambia el `isPro` y nada más: los mismos nueve sobres, el mismo costo.
  assert.equal(libre.respuestaDespues.snapshots.length, 9);
  assert.deepEqual(pago.respuestaDespues.snapshots, libre.respuestaDespues.snapshots);
  assert.deepEqual(pago.despues, libre.despues);
  assert.ok(libre.despues.documentos < libre.antes.documentos / 10);
});

test("el refresco Free persiste sólo lunarType y elementMap, y el alcance nuevo las vuelve a leer", async () => {
  const escenario = cuenta();
  const accion = ctxDeAccion(escenario);

  const publicado = await enElInstante<SobreDelDia>(INSTANTE, () =>
    (refreshForDateWithAccess as any)._handler(accion.ctx, { localDate: HOY, timezone: ZONA }),
  );

  // Lo que el refresco Free le pidió a `persistRefresh`: dos natales y nada más.
  assert.equal(accion.mutaciones.length, 1, "el refresco Free persiste una sola vez");
  assert.deepEqual(
    accion.mutaciones[0].results.map((resultado: AnalysisResult) => resultado.analysisId),
    ["ORB-LUN-001", "ORB-NAT-001"],
    "Free no puede persistir ninguna capa de pago",
  );
  assert.equal(accion.mutaciones[0].sky, null, "Free sale antes de pedir el cielo del día");

  // Lo que quedó escrito, leído de la tabla y no del argumento.
  const escritas = escenario.base.filas("analysisSnapshotsV492");
  assert.deepEqual(
    escritas.map((fila) => fila.analysisId).sort(),
    ["ORB-LUN-001", "ORB-NAT-001"],
  );
  assert.deepEqual(
    new Set(escritas.map((fila) => fila.localDate)),
    new Set([undefined]),
    "el natal se persiste sin fecha: cae en la franja que comparte cualquier día",
  );

  // El sobre que recibe Free sigue siendo el cerrado.
  for (const [nombre, tomar] of CERRADOS_PARA_FREE) {
    assert.deepEqual(tomar(publicado).missingInputs, ["orbita_plus"], `${nombre} cerrado`);
  }

  // Y el alcance nuevo vuelve a leer exactamente esas dos filas: lo que el
  // ciclo Free escribe es lo que el ciclo Free lee.
  escenario.base.reiniciarLecturas();
  const releido = await enElInstante(INSTANTE, () => leerDia(escenario, HOY, ZONA));
  const leido = materializadas(escenario);
  assert.deepEqual(
    new Set(leido.map((fila) => fila._id)),
    new Set(escritas.map((fila) => fila._id)),
  );
  assert.equal(
    releido.natal.lunarType.inputHash,
    escritas.find((fila) => fila.analysisId === "ORB-LUN-001")!.inputHash,
    "la fila persistida por el refresco Free es la que el alcance acotado recupera",
  );
});

test("refreshTransitArcWithAccess corta Free antes de calcular y no escribe nada", async () => {
  const escenario = cuenta();
  const accion = ctxDeAccion(escenario);

  const publicado = await enElInstante<AnalysisResult>(INSTANTE, () =>
    (refreshTransitArcWithAccess as any)._handler(accion.ctx, {
      localDate: HOY,
      timezone: ZONA,
      arcId: RANKING_TOP.arcId,
    }),
  );

  assert.equal(publicado.analysisId, "ORB-TRN-001");
  assert.deepEqual(publicado.missingInputs, ["orbita_plus"]);
  assert.equal(publicado.data, null);
  assert.equal(accion.mutaciones.length, 0, "el corte Free no persiste nada");
  assert.equal(escenario.base.filas("analysisSnapshotsV492").length, 0);
});
