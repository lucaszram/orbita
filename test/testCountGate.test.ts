...
test("el piso es el del brief más lo que sumaron CORE-191, el detalle del ranking, Vínculos, el panorama de Tránsitos la biblioteca de Vínculos y la estación vital: 970 tests", () => {
  ...
  assert.equal(DEFAULT_MINIMUM, 970);
});
...
test("el piso incluye su propio número: exactamente 970 pasa, 969 falla", () => {
  assert.equal(evaluateTestRun(resumen(970)).ok, true);

  const abajo = evaluateTestRun(resumen(969));
  assert.equal(abajo.ok, false);
  assert.match(abajo.failures[0], /969/);
