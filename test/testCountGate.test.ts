...
test("el piso es el del brief más lo que sumaron CORE-191, el detalle del ranking, Vínculos, el panorama de Tránsitos la biblioteca de Vínculos, la estación vital y el límite Free: 974 tests", () => {
  ...
  assert.equal(DEFAULT_MINIMUM, 974);
});
...
test("el piso incluye su propio número: exactamente 974 pasa, 973 falla", () => {
  assert.equal(evaluateTestRun(resumen(974)).ok, true);

  const abajo = evaluateTestRun(resumen(973));
  assert.equal(abajo.ok, false);
  assert.match(abajo.failures[0], /973/);
