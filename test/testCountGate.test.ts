...
test("el piso es el del brief más lo que sumaron CORE-191, el detalle del ranking, Vínculos, el panorama de Tránsitos, la biblioteca de Vínculos, la estación vital, el límite Free el tema del año, la Carta responsive los cuatro ritmos lo principal de Hoy, su ranking, la biblioteca y la comparación de Vínculos como el frame más el segmento de Tránsitos por URL y con Plus: 1021 tests", () => {
  ...
  assert.equal(DEFAULT_MINIMUM, 1021);
});
...
test("el piso incluye su propio número: exactamente 1021 pasa, 1020 falla", () => {
  assert.equal(evaluateTestRun(resumen(1021)).ok, true);

  const abajo = evaluateTestRun(resumen(1020));
  assert.equal(abajo.ok, false);
  assert.match(abajo.failures[0], /1020/);
