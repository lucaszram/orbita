...
/** Hora local a partir de la cual se puede sacar la carta del día (copy del Figma:
 *  "se abre a las 9:00"). Apagado por ahora: con esto en true no podés testear la app
 *  antes de las 9. Prenderlo cuando el ritual esté decidido. */
const CARD_UNLOCK_ENABLED = false;
const CARD_UNLOCK_HOUR = 9;
...
export const revealCard = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // No se puede sacar la carta de un día que no llegó.
    const today = localDateForTimezone(DEFAULT_TIMEZONE);
    if (args.localDate > today) throw new Error("Ese día todavía no llegó");

    if (CARD_UNLOCK_ENABLED && args.localDate === today) {
      const hour = Number(
        new Intl.DateTimeFormat("en-GB", { timeZone: DEFAULT_TIMEZONE, hour: "2-digit", hour12: false }).format(
          new Date()
        )
      );
      if (hour < CARD_UNLOCK_HOUR) throw new Error(`La carta se abre a las ${CARD_UNLOCK_HOUR}:00`);
    }
