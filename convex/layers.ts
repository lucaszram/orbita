 ...
 *
 * El alcance no sabe nada de planes: es el mismo para Free y para Órbita Plus.
 * El corte de plan vive donde vivía —en `getForDateForPlan`,
 * `getTransitArcForPlan` y `refreshForDateForPlan`, sobre estas mismas filas—,
 * así que acotar la lectura no mueve ni una gate.
 */
 ...
 *
 * Las tres familias se leen enteras también para Free: `lockedLayerBundle`
 * cierra `ORB-REL-001` DESPUÉS de que `natalResults` eligió, sobre el mismo
 * paquete natal que ve Órbita Plus. Leer menos acá cambiaría lo que el plan
 * cierra, que no es lo que acota esta tarjeta.
 */
