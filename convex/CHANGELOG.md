...
## 2026-09-18 — Las capas leen sólo el alcance del día: índice `by_user_local_date_timezone` (CORE-431)

- **Qué cambia (schema, aditivo):** `analysisSnapshotsV492` suma el índice `by_user_local_date_timezone` sobre `["userId", "localDate", "timezone"]`. `by_user`, `by_user_analysis` y `by_cache_key` quedan como estaban: el barrido completo por persona lo sigue necesitando el borrado de cuenta (`USER_SCOPED_DELETION_STEPS`), que tiene que llevarse TODO.
- **Qué cambia (lectura):** `layers.getForDate`, `layers.getNatalBase` y `layers.getRefreshState` ya no materializan todos los análisis de la persona para después descartar en memoria los de otras fechas. `getForDate` y `getRefreshState` leen tres rangos del índice nuevo —filas sin fecha ∪ (fecha pedida ∧ sin zona) ∪ (fecha pedida ∧ zona pedida)—, que es exactamente el conjunto que el filtro anterior dejaba pasar; `getNatalBase` lee por `by_user_analysis` las tres familias que el paquete natal consulta (`ORB-LUN-001`, `ORB-NAT-001`, `ORB-REL-001`). **El contrato público no cambia:** ningún sobre, ningún hash de caché y ninguna política de expiración se tocan.
- **Semántica conservada, explícita:** las filas legacy sin `localDate` —y también las que tienen zona pero no fecha— siguen entrando en el alcance de cualquier día; una fila del día que nunca declaró zona sigue entrando; el dato de OTRA zona del mismo día sigue sin servirse como si fuera el de acá; la selección sigue ganándola el `updatedAt` más alto, y el empate se resuelve con el orden del índice —primero `_creationTime` y, si también empata, el `_id`— igual que antes (ahora escrito en `bySelectionOrder`, porque las filas ya no llegan de un solo rango ordenado y la estabilidad del `sort` dejaría decidir al orden en el que se concatenan las franjas). El caché negativo vencido se sigue descartando, los dos arcos del mismo día siguen siendo dos filas y una fila natal legacy con fecha sigue siendo candidata del paquete natal.
- **Por qué:** el costo de abrir Hoy crecía con cada día que la cuenta hubiera guardado alguna vez, aunque la respuesta fuera siempre del mismo tamaño. Con el alcance acotado, agregar fechas o zonas ajenas no mueve una sola lectura.
- **Medición (LOCAL y sintética, no son los bytes que factura Convex):** `test/layersReadScope.test.ts` instrumenta los handlers reales contando los documentos que cada rango de índice materializa ANTES de cualquier filtro de aplicación, y su tamaño serializado, e imprime la comparación ANTES/DESPUÉS. Sobre una cuenta con 1.800 filas de fechas y zonas ajenas: `getForDate` 1.809 → 9 documentos, `getNatalBase` 1.809 → 3, `getRefreshState` 1.809 → 9. Sobre una cuenta SIN fechas acumuladas —donde no hay nada de otro día que descartar— `getForDate` y `getRefreshState` leen los mismos 9 documentos que antes, y `getNatalBase` baja igual de 9 a 3, porque el natal nunca necesitó más que sus tres familias. La medición real en DEV —versión desplegada, tasa de ejecución y ahorro facturado— exige un deploy aparte y no está incluida.
- **Sin cambios de front:** `LayersProvider` ya era un proveedor único con single-flight y dos suscripciones (hoy y ayer); las dos siguen igual y ahora cada una lee sólo su propio alcance.
...
- **Qué publica cada `Anillo`:** `key`, `label`, `cadence`, `state` (p. ej. `Nueva`, `Casa 6 · mes 10 de 12`, `Día 19,7 de 29,4`, `Luna con tu Marte · máxima precisión`), `status` (`ready` | `unavailable`), `precision`, `progressMode` (`point` con `progress`, `range` con `progressRange`, `unavailable`), `detail`, `available`, `failed` (sólo si la fuente falló hoy) y `limitations` (las de su propia fuente). El tránsito avanza por su posición dentro de la ventana (`transitNow` en la zona natal), no por su cercanía al exacto; sus rótulos de fase son los de release («se acerca», «máxima precisión», «sigue activo después del punto más preciso»). El sobre suma `exact` (hora natal exacta: avances en punto para estación, año y tránsito; el ritmo lunar es siempre franja porque su ventana tiene ancho real; sin hora exacta, todo son franjas), `availableCount`, `summary` y `observedAt`. Un ritmo sin cálculo no recibe un anillo estimado.
...
- **Front:** la tarjeta 02 del hub de Tu momento muestra casa, mes y regente reales; `/reading/tema-del-ano` según los frames `1741:2289` / `2024:2925` con la copy de Build 30 (`yearMeaning`, `yearReading` y la tabla de titulares `houseTheme`, portadas de `release/1.0.0` en `src/domain/momento.ts`; el titular es el mismo en el hub y en el detalle). La capa 03 sigue declarada pendiente (CORE-211).
