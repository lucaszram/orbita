...
- **Checkout:** `payments.createCheckoutSession({ plan })` cambia de `weekly | yearly | lifetime` a `weekly | yearly`. El anual configura siete días de trial. Se agrega `payments.getCheckoutStatus({ sessionId })` → `pending | active | failed`; valida que sesión, customer y Clerk user sean la misma cuenta y sólo devuelve `active` después del entitlement autoritativo del webhook. `payments.createPortalSession({})` exige una suscripción Stripe activa propia.
