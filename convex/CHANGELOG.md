...
- **Niveles:** `signo` (nombre + signo solar: sólo tono por elementos, sin contactos), `fecha` (fecha, hora y lugar opcionales: contactos entre planetas, sin ejes) y `carta` (fecha, hora y lugar obligatorios: suma Ascendente y Medio Cielo). La precisión que **se pudo** calcular viaja en `precision` (`level`, `label`, `includesAngles`, `limitations`): pedir `carta` con una carta propia sin hora baja a `fecha` y lo dice; no se finge un eje calculado al mediodía.
