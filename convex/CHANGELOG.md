...
- **Free y Órbita Plus, intactos:** el alcance de lectura no sabe nada de planes. `getForDateWithAccess`, `getTransitArcWithAccess`, `refreshForDateWithAccess` y `refreshTransitArcWithAccess` siguen exportados y siguen cortando donde cortaban (`enforcePlan`), `getRefreshState` sigue publicando `isPro`, `lockedLayerBundle` cierra las mismas capas y el refresco Free sigue persistiendo sólo `ORB-LUN-001` y `ORB-NAT-001`. El inventario de exports del módulo es el mismo antes y después.
