...
export default function HomeScreen() {
  ...
  const { status, isLive, auth, retrySession } = useLiveApp();
  // Mock SOLO para invitado CONFIRMADO. `booting`/`reconnecting` = carga estable:
  // una sesión existente no ve datos demo ni un segundo (incidente 2026-07-13).
  ...
  const userKey = isLive ? auth?.email ?? "user" : null;
  ...
  let revealed = guest ? guestRevealed : stripRevealed && daily?.carta != null;

  // ─── TESTING · SACAR ANTES DEL LAUNCH (junto con testingReplay.ts) ────────
  // Replay del primer día: la Home trata la carta de hoy como no sacada y sin
  // archivo previo, para que la intro y el primer flip se puedan re-vivir.
  const [, setReplayTick] = useState(0);
  const replaying = !guest && testingReplay.isArmed();
  if (replaying) revealed = false;
  // ─── FIN TESTING ───────────────────────────────────────────────────────────
  ...
  async function pullCard(): Promise<boolean> {
    ...
    // Replay: el server ya tiene el reveal de hoy; el flip es solo local.
    if (replaying && stripRevealed) {
      testingReplay.disarm();
      marcarPrimerRitual();
      setReplayTick((t) => t + 1);
      return true;
    }
  ...
  // (TESTING: durante el replay se fuerza 0 para re-vivir el día 1 puro.)
  const prevRevealed = replaying ? 0 : (strip ?? []).filter((d) => d.localDate !== today && d.revealed).length;
  ...
  const sessionPending = status === "booting" || status === "reconnecting";
      ...
      >
        ...
        {sessionPending ? (
          ...
          <View style={styles.loadingBlock}>
            ...
            <Pressable onPress={retrySession} accessibilityRole="button" style={styles.retryBtn}>
